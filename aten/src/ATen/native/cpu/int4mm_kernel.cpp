...
inline void ref_dyn_quant_matmul_4bit_channelwise_kernel(
    size_t m,
    size_t n,
    size_t k,
    const float* lhs_f32,
...
inline void ref_dyn_quant_matmul_4bit_groupwise_kernel(
    size_t m,
    size_t n,
    size_t k,
    size_t bl,
...
void dyn_quant_matmul_4bit_kernel(
  ...
  if (block_size == K / 2) {
    ref_dyn_quant_matmul_4bit_channelwise_kernel(
        M, N, K, lhs_f32, rhs_4bit, rhs_scales_f32, dst_f32, -FLT_MAX, FLT_MAX);
  } else if (32 == block_size || 64 == block_size) {
    ...
    TORCH_CHECK(
        block_size == K / 2 || block_size == 32 || block_size == 64,
        __func__,
        ": Group size should be 32, 64 or in_features/2 [",
        K / 2,
        "]. Provided ",
        block_size);
  }
