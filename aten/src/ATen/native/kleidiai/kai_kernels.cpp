...
void kai_pack_int4_rhs(
  ...
  if (bl == k / 2) {
    ...
    // Kleidiai channelwise kernel only accepts float32 scales
    Tensor float32_scales = scales;
    if (scales.dtype() == kHalf || scales.dtype() == kBFloat16) {
      float32_scales = scales.to(kFloat);
    }
    kai_pack_rhs_channelwise_int4<kai_matmul_ukernel_f32_qa8dxp_qs4cxp>(
        kernel_packet, weight_packed, weight, float32_scales, bias, n, k);
  } else if (32 == bl || 64 == bl) {
    // Groupwise
    ...
    Tensor bf16_scales = scales;
    // Kleidiai groupwise kernel only accepts bf16 scales
    if (scales.dtype() == kFloat || scales.dtype() == kHalf) {
      bf16_scales = scales.to(kBFloat16);
    }
    ...
    while (max_threads > 0) {
      ...
      if (m % num_thread_y == 0 && n % num_thread_x == 0) {
        ...
        if (m_per_thread % gemm_m_step == 0 &&
          ...
          division = {num_thread_y, num_thread_x, true};
          return division;
        }
      }
      max_threads -= 2;
...
static void kai_quant_pack_lhs_int4_mm_channelwise(
  ...
  auto gemm_kernel_packet = kai_select_channelwise_matmul_ukernel(gemm_id);
