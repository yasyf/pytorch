...
def get_kai_packed_weight_size(n_bits, N, K, groupsize):
    if n_bits == 4:
        if groupsize == K // 2:  # channelwise
        ...
        elif groupsize == 32 or groupsize == 64:  # groupwise
            kai_nr = 8
            kai_kr = 16
            kai_sr = 2
            ...
            def kai_get_rhs_packed_size_rhs_pack_nxk_qsi4c32p_qsu4c32s1s0(n, k, nr, kr, sr, bl):
                assert (bl % kr) == 0
                ...
                return num_rows * kai_get_rhs_packed_stride_rhs_pack_nxk_qsi4c32p_qsu4c32s1s0(k, nr, kr, sr, bl)

            def kai_get_rhs_packed_stride_rhs_pack_nxk_qsi4c32p_qsu4c32s1s0(k, nr, kr, sr, bl):
                assert (bl % kr) == 0
                assert (nr % kai_nr_multiple_of) == 0
                assert (bl % kai_bl_multiple_of) == 0

                # kr and sr are unused in the calculation
                num_bytes_multiplier_rhs = kai_get_datatype_size_in_bytes()
                num_blocks_per_row = kai_num_blocks_per_row(k, bl)
                num_bytes_per_block = kai_num_bytes_per_block(bl, num_bytes_multiplier_rhs)

                return nr * ((num_bytes_per_block * num_blocks_per_row) + kai_num_bytes_sum_rhs + kai_num_bytes_bias)

            def kai_get_datatype_size_in_bytes():
                return 0x0213 >> 8  # 2 bytes
...
def meta__dyn_quant_pack_4bit_weight(weights, scales_zeros, bias, block_size, in_features, out_features):
    torch._check(
    ...
    if torch.backends.kleidiai.is_available():
        torch._check(
            block_size == in_features // 2 or block_size == 32 or block_size == 64, lambda: "Group size must be a in_features/2 or 32 or 64"
        )
        packed_weight_size = get_kai_packed_weight_size(4, out_features, in_features, block_size)
...
def meta__dyn_quant_matmul_4bit(inp, packed_weights, scales_zeros, bias, block_size, in_features, out_features):
